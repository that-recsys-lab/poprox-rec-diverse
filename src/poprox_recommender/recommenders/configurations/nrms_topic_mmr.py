...
def configure(builder: PipelineBuilder, num_slots: int, device: str):
    ...
    n_reranker = builder.add_component(
        ...
        candidate_articles=n_scorer,  # Articles with relevance scores
        interest_profile=e_user,  # User embedding for diversity calculation
    )

    # Input: All candidates + user profile → Output: Topic-aligned candidates
    # Creates a TopicFilter component that filters candidate articles based on the user's topic interests
    # Takes the full candidate set (i_candidates) and user profile (i_profile) as inputs
    # Returns only articles that match the user's declared topic preferences
    ...
    # candidates1: Topic-filtered articles (from line 1)
